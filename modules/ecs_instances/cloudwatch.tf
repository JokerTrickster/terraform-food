...
resource "aws_cloudwatch_log_group" "dmesg" {
  name              = "${var.project}/var/log/dmesg"
  retention_in_days = 30
}

resource "aws_cloudwatch_log_group" "docker" {
  name              = "${var.project}/var/log/docker"
  retention_in_days = 30
}

resource "aws_cloudwatch_log_group" "ecs-agent" {
  name              = "${var.project}/var/log/ecs/ecs-agent.log"
  retention_in_days = 30
}

resource "aws_cloudwatch_log_group" "ecs-init" {
  name              = "${var.project}/var/log/ecs/ecs-init.log"
  retention_in_days = 30
}

resource "aws_cloudwatch_log_group" "audit" {
  name              = "${var.project}/var/log/ecs/audit.log"
  retention_in_days = 30
}

resource "aws_cloudwatch_log_group" "messages" {
  name              = "${var.project}/var/log/messages"
  retention_in_days = 30
}

resource "aws_cloudwatch_log_group" "ecs-service" {
  name              = "ecs-idea-dev_food-recommendation"
  retention_in_days = 7
}